<x-main.app>
    @section('title', 'Home - '.$business->name)
    ...
    <div class="flex-1 max-w-7xl mx-auto py-4 md:p-4">
        <div class="grid grid-cols-1 gap-2 md:grid-cols-4">
            <div class="col-span-1 bg-white rounded shadow">
                <h1 class="text-sm font-bold text-gray-900 rounded-t bg-gray-50 mb-2 p-4 uppercase">
                    Categories
                </h1>
                <div class="flex flex-col">
                    <a href="{{ route('home') }}" class="p-4 hover:bg-yellow-900 hover:bg-opacity-75 hover:text-white border-b {{ ($category == 'All') ? 'text-yellow-900 font-semibold shadow' : '' }}">All</a>
                    @foreach ($productCategories as $productCategory)
                        <a href="{{ route('home') }}?category={{ $productCategory->name }}" class="p-4 hover:bg-yellow-900 hover:bg-opacity-75 hover:text-white border-b {{ ($category == $productCategory->name) ? 'text-yellow-900 font-semibold shadow' : '' }}">{{ $productCategory->name }}</a>
                    @endforeach
                </div>
            </div>
            <div class="col-span-1 md:col-span-3 bg-white rounded shadow">
                <div class="rounded-t bg-gray-50 p-4">
                    <h1 class="text-sm uppercase text-gray-900 font-bold mr-2 flex items-center gap-2">
                        Products
                        <small class="text-yellow-900 flex items-center justify-center gap-2">
                ...
                <div class="px-4 pt-4">
                    <p class="text-green-500 text-sm">{{ $products->total() }} result(s) for <span class="font-semibold">"{{ $category }} Category"</span>!</p>
                </div>
